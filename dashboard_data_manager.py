...
def update_and_get_dashboard_state(optimization_results, initial_train_data):
    ...
    # ==========================================================================
    # ** THE FIX IS HERE: Sanitize the input data first **
    # Convert all time strings from the initial request into datetime objects
    # ==========================================================================
    ...
    for rec in recommendations:
        initial_info = sanitized_initial_data.get(rec['train_id'], {})
        audit_data.append({
            'id': f"AUD_{str(uuid.uuid4().hex)[:6].upper()}",'trainId': rec['train_id'],
