...
# ==============================================================================
# 1. CORE CLASSES (DelayFactors, NetworkTimeState & TrainJourney)
# ==============================================================================

class DelayFactors:
    """Represents external, unpredictable delays for a train."""
...
class NetworkTimeState:
    """Defines the static railway network infrastructure."""
    def __init__(self):
        self.edge_travel_times = {
            ('Entry_1', 'A'): 3, ('Entry_4', 'A'): 2, ('Entry_2', 'B'): 4,
            ('Entry_5', 'B'): 3, ('Entry_3', 'C'): 5, ('Entry_6', 'C'): 3,
            ('F', 'Entry_10'): 3, ('F', 'Entry_12'): 3, ('E', 'Entry_9'): 4,
            ('E', 'Entry_11'): 2, ('D', 'Entry_7'): 2, ('D', 'Entry_8'): 4,
            ('A', 'P1_entry'): 2, ('P1_exit', 'F'): 2,
            ('B', 'P2_entry'): 2, ('P2_exit', 'E'): 2,
            ('C', 'P3_entry'): 2, ('P3_exit', 'D'): 2,
            ('A', 'B'): 1.5, ('B', 'A'): 1.5, ('E', 'F'): 1.5, ('F', 'E'): 1.5,
            ('P1_entry', 'P1_exit'): 5, ('P2_entry', 'P2_exit'): 5, ('P3_entry', 'P3_exit'): 5,
        }
        self.dwell_times = {'Passenger': 3, 'Special': 2, 'Freight': 8, 'Local': 5}

class TrainJourney:
    """Represents a train and calculates its valid, possible paths."""
...
TRAIN_PRECEDENCE = { 'Special': 4, 'Passenger': 3, 'Local': 2, 'Freight': 1 }

# ==============================================================================
# 2. HELPER FUNCTIONS (Pathfinding)
# ==============================================================================

def create_graph_from_data(network_state, non_functional_segments=None):
    """Builds a traversable graph, excluding non-functional tracks."""
    G = nx.DiGraph()
...
def find_all_possible_paths(graph, start_node, end_node):
    """Finds all simple paths (limited to 5 for performance)."""
    try:
...
# ==============================================================================
# 3. THE CORE OR ALGORITHM
# ==============================================================================

class PathBasedSolution:
    """Represents a complete solution: an action and a path for every train."""
...
def calculate_objective_cost(solution, network_state):
    """An event-based simulation that handles both track and node contention."""
    ...
    while event_queue:
        ...
        last_track_exit = track_occupancy.get(segment, datetime.min)
        last_node_exit = node_occupancy.get(start_node, datetime.min)
        ...
        track_occupancy[segment] = exit_time
        node_occupancy[end_node] = exit_time
        ...
        if decision['action'] == 'PROCEED' and decision['path']:
            ...
            for other_tid, finish_time in actual_finish_times.items():
                if TRAIN_PRECEDENCE.get(solution.train_journeys[other_tid].train_type, 0) >= TRAIN_PRECEDENCE.get(journey.train_type, 0):
                    if finish_time > latest_clear_time:
                        latest_clear_time = finish_time
            held_delay = (latest_clear_time - journey.actual_arrival_time).total_seconds() / 60
            individual_delays[tid] = max(0, held_delay)
            total_delay += individual_delays[tid]

    return total_delay - throughput_bonus, individual_delays

def generate_neighbor(solution):
    """Creates a new solution by making a small random change."""
    ...
    if random.random() < 0.5:
        ...
        if neighbor.decisions[tid]['action'] == 'PROCEED':
            neighbor.decisions[tid]['action'] = 'HOLD'
            neighbor.decisions[tid]['path'] = None
        elif neighbor.train_journeys[tid].possible_paths:
            neighbor.decisions[tid]['action'] = 'PROCEED'
            neighbor.decisions[tid]['path'] = neighbor.train_journeys[tid].default_path
    return neighbor
...
def simulated_annealing(train_journeys, network_state, iterations=2000, temp=1000, cool_rate=0.99):
    ...
    current_cost, _ = calculate_objective_cost(current_sol, network_state)
    best_sol, best_cost = current_sol, current_cost
    for _ in range(iterations):
        if temp <= 0.01: break
        neighbor = generate_neighbor(current_sol)
        neighbor_cost, _ = calculate_objective_cost(neighbor, network_state)
...
def execute_module(train_data, non_functional_segments=None):
    """Main function to run the optimization and report results."""
    ...
    for tid, info in train_data.items():
        ...
        journey_info['scheduled_entry_time'] = datetime.fromisoformat(journey_info['scheduled_entry_time'])
        journey_info['scheduled_exit_time'] = datetime.fromisoformat(journey_info['scheduled_exit_time'])
    ...
    _, final_delays = calculate_objective_cost(best_solution, network_state)
    ...
    return {'score': round(final_score, 2), 'recommendations': recommendations}



def get_simulation_timeline(solution, network_state):
    """
    Runs a detailed simulation for a final, optimal solution to get the
    exact timeline for every train on every segment.
    Returns a dictionary: {train_id: {segment: (start_time, end_time)}}
    """
    track_occupancy, node_occupancy = {}, {}
    ...
    for tid, decision in solution.decisions.items():
        if decision['action'] != 'HOLD' and decision['path']:
            heapq.heappush(event_queue, (solution.train_journeys[tid].actual_arrival_time, tid, 0))

    while event_queue:
        current_time, train_id, path_idx = heapq.heappop(event_queue)
        path = solution.decisions[train_id]['path']
        if path_idx >= len(path) - 1: continue
        segment = (path[path_idx], path[path_idx + 1])
        start_node, end_node = segment
        last_track_exit = track_occupancy.get(segment, datetime.min)
        last_node_exit = node_occupancy.get(start_node, datetime.min)
    ...
    return actual_timelines

def track_train_position(solution, network_state, train_id, query_time):
    """
    Finds the exact position of a specific train at a given time
    based on the final, optimized schedule.
    """
    decision = solution.decisions[train_id]
    journey = solution.train_journeys[train_id]

    if decision['action'] == 'HOLD':
        return f"Train {train_id} is HELD at {journey.entry_node}."

    if query_time < journey.actual_arrival_time:
        return f"Train {train_id} has not yet started. Scheduled to depart at {journey.actual_arrival_time.strftime('%H:%M:%S')}."

    timelines = get_simulation_timeline(solution, network_state)
    train_timeline = timelines.get(train_id, {})

    for segment, (start, end) in train_timeline.items():
        if start <= query_time < end:
            return f"Train {train_id} is currently ON TRACK from {segment[0]} to {segment[1]}."

    if not train_timeline or (train_timeline and query_time >= max(end for start, end in train_timeline.values())):
        return f"Train {train_id} has COMPLETED its journey."

    return f"Status of Train {train_id} is UNKNOWN at this time."